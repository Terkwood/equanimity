...
const SLEEP_KEY: &str = "sleep";
    ...
    pub fn save_mood_readings( all: &Vec<MoodReading>) -> Result<(), StorageError> {
        LocalStorage::set(MOOD_READINGS_KEY, serde_json::to_string(all).expect("readings to string"))
    }
    ...
    pub fn save_text(
        ...
        LocalStorage::set(text_key(text_type), serde_json::to_string(all).expect("text subs to string"))
    }
